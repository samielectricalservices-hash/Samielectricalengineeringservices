    ...
    <AppShell>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        ...
        <aside className="space-y-6">
          <Panel title="Status">
            <Info label="Current Status" value={repair.status?.replace("_", " ")} />
            <Info label="Priority" value={repair.priority?.replace("_", " ")} />
          ...
          <Panel title="Costs">
            ...
            <Info label="Profit" value={money(repair.cost?.profit)} />
            <Info label="Payment Status" value={repair.cost?.paymentStatus?.replace("_", " ")} />
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="rounded-lg border bg-card p-5 shadow-sm"><h2 className="mb-4 text-base font-semibold">{title}</h2><div className="grid gap-4 sm:grid-cols-2">{children}</div></section>;
}
