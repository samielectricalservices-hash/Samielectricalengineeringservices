    ...
    <AppShell>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {Object.entries(summary).map(([period, value]) => (
          <section key={period} className={`business-card rounded-lg border p-5 ${period === "annual" ? "card-purple" : period === "monthly" ? "card-blue" : period === "weekly" ? "card-teal" : "card-emerald"}`}>
            ...
            <StatusBadge type="profit" value={`${money.format(value.profit)} Birr`} amount={value.profit} className="mt-4" />
          </section>
        ))}
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {reports.map((report) => (
          <section key={report} className="business-card rounded-lg border p-5">
      ...
      <section className="business-card mt-6 overflow-hidden rounded-lg border">
        ...
        {overview.recent.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-sm">
              <thead className="calm-table-head text-left">
                <tr>
              ...
              <tbody>
                {overview.recent.map((repair) => (
                  <tr key={repair.id} className={`border-t ${repair.status === "COMPLETED" ? "row-completed" : repair.status === "OPEN" ? "row-pending" : repair.status === "CANCELLED" ? "row-urgent" : ""}`}>
                    ...
                    <td className="px-5 py-3"><StatusBadge value={repair.status} /></td>
                    <td className="px-5 py-3 text-right">{money.format(repair.finalAmount)}</td>
                    <td className="px-5 py-3 text-right font-medium"><StatusBadge type="profit" value={money.format(repair.profit)} amount={repair.profit} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {rows.length === 0 ? (
          ...
          <div key={row.label}>
            ...
            <div className="h-2 rounded bg-secondary">
              <div className="h-2 rounded bg-accent" style={{ width: `${Math.max(4, row.value / max * 100)}%` }} />
            </div>
          </div>
        ))}
      </div>
